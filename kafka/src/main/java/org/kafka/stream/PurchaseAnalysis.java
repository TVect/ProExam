...
import org.kafka.stream.model.Item;
import org.kafka.stream.model.Order;
import org.kafka.stream.model.User;
import org.kafka.stream.serdes.SerdesFactory;
import org.kafka.stream.timeextractor.OrderTimestampExtractor;
...
public class PurchaseAnalysis {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		KStream<String, Order> orderStream = streamBuilder.stream(Serdes.String(), 
												SerdesFactory.serdFrom(Order.class), "orders");
		KTable<String, User> userTable = streamBuilder.table(Serdes.String(), 
											SerdesFactory.serdFrom(User.class), "users", "users-state-store");
		KTable<String, Item> itemTable = streamBuilder.table(Serdes.String(), 
											SerdesFactory.serdFrom(Item.class), "items", "items-state-store");
//		itemTable.toStream().foreach((String itemName, Item item) -> System.out.printf("Item info %s-%s-%s-%s\n", item.getItemName(), item.getAddress(), item.getType(), item.getPrice()));
		KTable<Windowed<String>, Double> kTable = orderStream
				.leftJoin(userTable, 
				...
				.leftJoin(itemTable, 
						(OrderUser orderUser, Item item) -> OrderUserItem.fromOrderUser(orderUser, item), 
						Serdes.String(), 
						SerdesFactory.serdFrom(OrderUser.class))
...
//				.map((String item, OrderUserItem orderUserItem) -> KeyValue.<String, Double>pair(orderUserItem.gender, (Double)(orderUserItem.quantity * orderUserItem.itemPrice)))
				.map((String item, OrderUserItem orderUserItem) -> KeyValue.<String, Double>pair(item, (Double)(orderUserItem.quantity * orderUserItem.itemPrice)))
				.groupByKey(Serdes.String(), Serdes.Double())
				.reduce((Double v1, Double v2) -> v1 + v2, 
						TimeWindows.of(36000L).advanceBy(5000L),
						"gender-amount-state-store");
		kTable.foreach((str, dou) -> System.out.printf("%s-%s\n", str, dou));
		kTable.to("item-amount");
//			.toStream()
