package org.kafka.stream.producer;
...
import org.kafka.stream.partitioner.HashPartitioner;
import org.kafka.stream.serdes.GenericSerializer;
...
public class OrderProducer {
	...
	public static List<Order> readOrder() throws IOException {
		...
		List<Order> orders = lines.stream()
			...
			.map((String[] values) -> new Order(values[0], values[1], LocalDateTime.parse(values[2], dataTimeFormatter).toEpochSecond(ZoneOffset.UTC) * 1000, Integer.parseInt(values[3].trim())))
